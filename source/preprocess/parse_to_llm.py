...
def read_micro_service(service_name, version="v1"):
    ...
    for f in json_files:
        ...
        for k, v in r.items():
            # v is a list of all verbs
            for verb in v:
                r = {'service': service_name, 'resource': k, 'verb': verb[0], 'request': ",".join(verb[1]), 'response': ",".join(verb[2])}
                json_result.append(r)
    return json_result

def main(result_file="cloudforet_api_v1_train.json", test_file="cloudforet_api_v1_test.json", random_seed=10):
